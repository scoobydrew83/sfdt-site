...
| `releaseNotesDir` | string | Release-notes output directory (default `release-notes`) — project-relative only ([why](#repo-supplied-settings-that-need-an-opt-in)) |
| `manifestDir` | string | Manifest output directory (default `manifest/release`) — project-relative only ([why](#repo-supplied-settings-that-need-an-opt-in)) |
| `manifestLayout` | enum | `flat` (default) or `subpath` for per-package subdirectories |
| `changelogDir` | string | Per-package changelog directory (default `changelogs`) — project-relative only ([why](#repo-supplied-settings-that-need-an-opt-in)) |
| `logDir` | string | CLI log directory (default `logs`) — project-relative only ([why](#repo-supplied-settings-that-need-an-opt-in)) |
...
| `plugins` | array | npm package names to load as plugins — **requires `SFDT_ALLOW_UNSAFE_CONFIG=1` since 0.22.0** ([why](#repo-supplied-settings-that-need-an-opt-in)) |
...
<Callout type="warning">
  **Path keys must be project-relative — since 0.24.0.** `releaseNotesDir`, `manifestDir`,
  `changelogDir`, `logDir` and their siblings are refused if the value is **absolute**, even when
  it resolves *inside* the project root, and the check now runs at config load rather than only in
  the dashboard. Before 0.24.0 an absolute path that happened to sit inside the project was
  accepted. On upgrade, a committed `"logDir": "/Users/me/myproject/logs"` is refused and replaced
  with the default — which changes where logs are written. Use `"logs"`, or set
  `SFDT_ALLOW_UNSAFE_CONFIG=1` if you trust the project.
</Callout>
...
| `mcp.salesforce.command` / `args` | Spawned as a process. **Since 0.25.0** either half alone triggers the refusal — `args` without `command` still applied attacker-chosen arguments to the default `sf` binary — and `mcp.salesforce` accepts **only** `transport`, `command` and `args`; any other key now fails config load rather than being silently ignored |
...
<Callout type="info">
  The opt-in is an **environment variable, not a config key** — by necessity. A flag inside
...
<Callout type="info">
  Since **0.23.1** the loopback test parses the URL and compares the resolved host, rather than
  pattern-matching the string. A URL carrying userinfo is refused outright:
  `http://127.0.0.1:80@example.invalid/v1` names `example.invalid` as its real host, and previously
  read as loopback — which mattered because the exemption is what keeps the `ai.apiKeyEnv`
  Authorization header attached. Ordinary local endpoints such as `http://127.0.0.1:11434/v1` are
  unaffected.
</Callout>
...
| `soql.defaultLimit` | `200` | Default row bound for [`sfdt soql query`/`sosl`](/cli/commands/metadata#sfdt-soql) when `--limit` isn't passed |
...
<Callout type="info">
  ...
  CLI and the Chrome extension's live [`org-health`](/chrome-extension/workspace) checks band
  org-health usage identically (shared via `@sfdt/flow-core`).
</Callout>

### `ci`

Defaults for [`sfdt ci init`](/cli/ci-cd#generating-a-pipeline--sfdt-ci-init). Flags override
...
header. See [AI Providers](/cli/ai-providers#gateway-headers).
