...
| **Scratch** | Scratch-org pool fill and active scratch orgs | `/api/scratch` |
| **Data** | Defined `sf data tree` data sets | `/api/data` |
| **Docs** | Docs config: output dir, diagrams, AI authoring | `/api/docs` |
...
The Test Runs, Preflight, Drift, Compare, and Pull pages have **Run** buttons that trigger the
underlying command over a Server-Sent Events stream — output appears line-by-line in real time,
exactly like running the CLI.
