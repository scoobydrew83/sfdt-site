...
A data set arrives with a clone, so `load` refuses symlinks in it. A source CSV must physically live
inside the set, and the field-mapped copy it writes to `.mapped/` cannot be redirected by a link
(since 0.26.2).
