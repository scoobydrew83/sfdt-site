...
description: audit, monitor, docs, data, scratch, and notify — diagnose org health, back up metadata, generate docs, seed data, manage scratch orgs, and push notifications to Slack, Teams, email, webhook, or Loki.
---
...
| `mfa-readiness` | Active users not ready for Salesforce's phishing-resistant MFA enforcement (July 2026) |
| `soap-logins` | SOAP `login()` traffic on API versions 31–64 (retiring Summer '27) |
...
| `org-info` | Instance, edition, and trial/expiration window (`monitoring.orgInfoTrialWarnDays`) |
...
A durable, queryable index of past runs, so org-health/coverage/deploy outcomes can be **trended**
over time instead of only showing the latest snapshot. Every audit/monitor/quality/test/deploy/
rollback/agent-test run is recorded to a compact SQLite index (`logs/history.db`), and audit/monitor
also archive full timestamped snapshots. Recording is best-effort — a history failure never fails
the command.

```bash
sfdt history                       # recent runs across all types
sfdt history --type audit          # only audit runs
...
| Option | Description |
|---|---|
| `--type <type>` | Filter to one run type (`audit`, `monitor`, `quality`, `test`, `deploy`, `rollback`, `agent-test`, …) |
...
channels (Slack, Microsoft Teams, email, generic webhook, or Grafana Loki).
