...
<Callout type="info">
  `--tag`, `--create-pr`, and `--notify` apply to the **standard manifest deploy only** — combining
  them with `--smart`, `--managed`, or `--source-dir` prints a warning instead of silently doing
  nothing.
</Callout>
