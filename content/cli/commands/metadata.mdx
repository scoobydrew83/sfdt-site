...
sfdt dependencies AccountTriggerHandler --gaps                # source-parsed inferred edges (offline)
sfdt dependencies AccountTriggerHandler --gaps --org uat      # diff against the org: MISSING vs confirmed
```
...
| `--gaps` | Report **source-parsed** references the Tooling API misses — dynamic Apex, LWC `@salesforce/apex`, formula and Flow references — by parsing local source offline. With `--org`, each inferred edge is diffed against the org and marked MISSING vs confirmed |
| `--org <alias>` | Target org (defaults to `config.defaultOrg`) |
| `--json` | Structured output |
