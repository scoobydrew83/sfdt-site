...
**v1.3**). Request kinds include `ping`, `version`, `deploy`, `rollback`, `quality`, `ai`,
`scan`, `compare`, `drift`, `org-health`, `telemetry.snapshot`, and the read-only manifest
builder pair `manifest.discover` / `manifest.render` (org type/member discovery and
package.xml / destructiveChanges.xml rendering — both also served by the native host).

- **Minor bump** (e.g. 1.2 → 1.3) — additive and backward-compatible (a new kind, a new
