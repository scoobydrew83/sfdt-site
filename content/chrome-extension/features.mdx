...
<Callout type="warning">
  **Off by default.** It deletes org records, so the control does not exist until you switch
  `soql-bulk-delete` on in the [options page](/chrome-extension/settings), where its row carries an
  **Off by default** badge. The button is not built, not merely hidden. It is one of two features
  that ship switched off — the other is [Generate query](#generate-query-soql-nl-generate).
</Callout>
...
**There is no cap on N.** One run deletes every loaded row the plan accepts — the only ceiling is
how many rows the runner is holding, and **Load more** will stack up to ten pages. The count in the
button and in the typed phrase is the only thing standing between you and that number.
...
<Callout type="warning">
  **What the backup guarantee actually is.** Two things are checked before the confirm appears, and
  failing either deletes nothing and never asks you: the **serialised CSV must contain every record
  Id in the plan** (a search of the text for each Id, so an empty or truncated file cannot pass),
  and the **browser must mint a blob URL** for it so the download can be handed off. That second
  check is the weaker one — it proves the handoff was started, not that it finished.

  What SFDT **cannot** verify is that the file reached your disk. Nothing in the extension can:
  that needs the `downloads` [permission](/chrome-extension/privacy), which is not in the manifest.
...
The last [`sfdt quality`](/cli/commands/testing-quality) run, in the
[Workspace](/chrome-extension/workspace). Salesforce Code Analyzer findings arrive over the
[bridge](/chrome-extension/bridge) and render as a per-file issue list — severity, line, rule id,
message, and engine attribution (pmd, eslint, …) — grouped by file worst-first and filterable by
both severity and engine. Files that map to a Setup component get **Open in Setup**, which
resolves the record Id through the Tooling API and deep-links to the component itself, falling
back to the type's Setup list rather than a dead link when the org has no matching row.

**A skipped scan is never shown as a pass.** If the analyzer did not run, the panel says so
instead of rendering an empty, reassuring result set.

Requires the bridge — the findings come from a CLI run, not from the org.
...
| `deploy-status` | Recent metadata deployments (`DeployRequest`): status, who, start/finish, component and test counts, validate vs deploy. Auto-refreshes while any deploy is in progress. Click a failed row for SOAP component errors. Degrades to a warning if the org refuses the query — no new permission |
