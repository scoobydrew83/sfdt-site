...
1. **A backup CSV downloads automatically** — exactly the rows about to be deleted and nothing else,
   serialised by the same code as **Export CSV**, so it quotes commas and quotes identically to the
   export you already use.
2. **A modal asks you to type the phrase exactly** — `DELETE 12 Account`. Confirm stays disabled
   ...
   not read.
3. **Only then** does anything reach the org, in waves of 25 with a live progress count.

If the backup cannot be written, nothing is deleted and you are never even asked. A failed row does
not abort the run: every row that did not delete is listed afterwards **by Id, with its reason**. A
row that **timed out** is called out separately as *may still have committed* — the request never
came back, so re-run the query to see what is actually left rather than retrying blind and deleting
something twice.

The table on screen is stale the moment a delete succeeds, and the status line says so rather than
leaving you to guess. Deletes follow the runner's **REST / Tooling** toggle, and add **no new
extension permission** — they go through the same worker-proxied session every other write already
uses.
...
| `soql-bulk-delete` | **Off by default.** Adds **Delete N rows** to the SOQL Runner's result toolbar when the rows carry an `Id`: automatic backup CSV → typed `DELETE <n> <Object>` confirmation → batched delete with progress and a per-row failure report. See [Highlights](#bulk-delete-from-a-soql-result-set-soql-bulk-delete) |
