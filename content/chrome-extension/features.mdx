---
title: Features
description: All 39 opt-in features of the SFDT SF Helper Chrome extension, grouped by the Salesforce context where they run.
---
...
| Id | What it does |
| --- | --- |
| `soql-runner` | Run SOQL (REST or Tooling) with field/object autocomplete, history, a **🔎 Explain** query-plan view (cost/cardinality/leading operation, no execution), **Copy JSON / Copy for Excel (TSV)**, CSV export of the current page **or all rows** (follows `queryMore` pagination with a progress indicator + cancel), and a LangGraph node generator |
...
| Id | What it does |
| --- | --- |
| `apex-anonymous` | Execute anonymous Apex with compile/runtime results, saved snippets, and a **DebugLevel picker** for the captured log |
| `debug-log-viewer` | List recent `ApexLog` records and view full log bodies, with a 15s **auto-refresh** toggle and **Delete all logs** (bulk-clears the org's logs behind a count-confirm) |
