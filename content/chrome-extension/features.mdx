...
it, badging each entry `SOQL` or `SOSL`. In SOSL mode the Tooling toggle and **Explain** are
disabled — the Search resource has no Tooling variant and no query plan.
...
| Id | What it does |
| --- | --- |
| `soql-runner` | Run **SOQL or SOSL** (a `SOQL \| SOSL` toggle that also auto-follows a `FIND {…}` query) with field/object autocomplete (fed fields from the Schema Browser's **Insert into query**), history, a **🔎 Explain** query-plan view (cost/cardinality/leading operation, no execution), **Copy JSON / Copy for Excel (TSV)**, CSV export of the current page **or all rows** (follows `queryMore` pagination with a progress indicator + cancel), and a LangGraph node generator. SOSL results are grouped per returned object, each group with its own copy/export toolbar — see [Highlights](#sosl-mode-in-the-soql-runner) |
