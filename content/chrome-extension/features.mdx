...
| **inferred** | A best-effort signal, *not* proof of a write — an Apex text hit (it may only read the field), a candidate flow that wasn't analysed, or a Flow write whose object couldn't be pinned down. |

An inferred hit is never shown as confirmed, and the confirmed/inferred counts are reported
separately. Scan limits are stated rather than hidden: a **standard** field has no dependency edge
to narrow by, so Flow coverage falls back to the most recently modified active flows and says so in
a **Scan scope** note — as do per-scan caps and any query the org refuses. Each row carries an
**Open** link into Flow Builder or the relevant Setup page.
